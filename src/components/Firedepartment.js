...
const Firedepartment = () => {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        ...
        const response = await axios.get('https://node-application-36uh.onrender.com/getAllRecords', {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
  ...
  const fetchSearchResults = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://node-application-36uh.onrender.com/search?query=${encodeURIComponent(searchQuery)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const handleDelete = async (uid) => {
    ...
    try {
      const token = localStorage.getItem('idToken');
      await axios.delete(`https://node-application-36uh.onrender.com/DNR/${uid}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
